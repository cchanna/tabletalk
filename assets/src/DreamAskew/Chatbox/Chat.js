            ...
            formatted = unformatted
              .replace(/{array}/g, () => {
                if (data.value.length === 3)
                  return `Chose "${data.value[0]}", "${data.value[1]}", and "${
                    data.value[2]
                  }" for`;
                if (data.value.length === 2)
                  return `Chose "${data.value[0]}" and "${data.value[1]}" for`;
                if (data.value.length === 1)
                  return `Chose "${data.value[0]}" for`;
                if (data.value.length === 0)
                  return `Unselected their choice for`;
              })
              .replace(/{(.*?) \? (.*?) : (.*)}/g, (_m, key, t, f) =>
                data[key] ? t : f
              )
