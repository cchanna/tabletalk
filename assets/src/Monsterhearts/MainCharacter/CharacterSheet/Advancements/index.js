...
export const remove = ({ id, advancementId }) => (dispatch, getState) => {
  const state = getState();
  const { addingStat } = fromMonsterhearts.getCharacter(
    state,
    id
  ).mainCharacter;
...
const mapStateToProps = (state, { id, depth }) => {
  ...
  const advancements = getPlaybookAdvancements(state, playbook).map(
    advancement => ({
      ...advancement,
  ...
  if (getCanGetSeasonAdvancements(state, id)) {
    seasonAdvancements = getSeasonAdvancements(state).map(advancement => ({
      ...advancement,
